...
class IpTestController implements ContainerInjectableInterface
{
     ...
     * @return object
     */
    public function ipAction() : object
    {
        $title = "Validera IP";
        ...
        $page->add("in-test/validator", $data);
     ...
     * @return object
     */
    public function ipActionPost() : object
    {
        $title = "Validera IP";
        ...
        $ip = $request->getPost("ininput", null);


        $session->set("dice", new Iptest($playerName, $noDice, $noSides));
        $dice = $session->get("dice");
        var_dump($ip);

        return $response->redirect("ip-test/validator");
    }
